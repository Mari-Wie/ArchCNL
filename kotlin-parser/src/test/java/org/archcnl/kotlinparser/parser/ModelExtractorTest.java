...
public class ModelExtractorTest {
  ...
  void givenExampleKotlinProject_whenModelExtractorVisitProject_thenAllSourceFilesFound() {
    ...
    final SourceFile interfaceSourceFile = sourceFiles.stream()
        .filter(f -> isTypeInFileDefined(f.getDefinedTypes(), interfaceType)).findAny().get();
    Assert.assertEquals(interfaceSourceFile.getNamespace().getName(), baseNamespace);

    final SourceFile getSourceFile = sourceFiles.stream()
        .filter(f -> isTypeInFileDefined(f.getDefinedTypes(), getType)).findAny().get();
    Assert.assertEquals(getSourceFile.getNamespace().getName(), baseNamespace);

    final SourceFile complexSourceFile = sourceFiles.stream()
        .filter(f -> isTypeInFileDefined(f.getDefinedTypes(), complexClass)).findAny().get();
    Assert.assertEquals(complexSourceFile.getNamespace().getName(), baseNamespace);

    final SourceFile sourceFileWithInnerClass = sourceFiles.stream()
        .filter(f -> isTypeInFileDefined(f.getDefinedTypes(), classWithInnerClass)).findAny().get();
    Assert.assertEquals(sourceFileWithInnerClass.getNamespace().getName(), baseNamespace);
    Assert.assertTrue(sourceFileWithInnerClass.getDefinedTypes().stream()
        .map(DefinedType::getSimpleName).collect(Collectors.toList()).contains(innerClass));

    final SourceFile sourceFileInSubpackage = sourceFiles.stream()
        .filter(f -> isTypeInFileDefined(f.getDefinedTypes(), classInSubpackage)).findAny().get();
    Assert.assertEquals(sourceFileInSubpackage.getNamespace().getName(),
        baseNamespace + ".subpackage");

    final SourceFile sourceFileInAnotherPackage = sourceFiles.stream()
        .filter(f -> isTypeInFileDefined(f.getDefinedTypes(), anotherClass)).findAny().get();
    Assert.assertEquals(sourceFileInAnotherPackage.getNamespace().getName(),
  ...
  private boolean isTypeInFileDefined(final List<DefinedType> definedTypes, final String typeName) {
