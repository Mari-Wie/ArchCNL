...
public class ArchiecturalRulesSelectedEvent extends ComponentEvent<ArchitecturalStyleRuleSelection> {
	...
	public ArchiecturalRulesSelectedEvent(ArchitecturalStyleRuleSelection source, boolean fromClient, ArchitecturalStyleConfig config, Set<String> selectedRules, Tabs tabs) {
	...
	public void handleEvent() {
		...
		for (ArchitectureRuleConfig ruleConfig : config.getRules()) {
			...
			if(selectedItems.contains(ruleToCheck)) { // if the rule is within the selected items
				...
				List<ArchitectureInformation> variableArchitectureInfomation = config.getVariableParts();
