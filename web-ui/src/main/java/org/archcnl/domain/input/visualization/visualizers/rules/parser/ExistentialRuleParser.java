...
import org.archcnl.domain.input.visualization.visualizers.rules.ExistentialRuleVisualizer;
import org.archcnl.domain.input.visualization.visualizers.rules.RuleVisualizer;

public class ExistentialRuleParser extends RuleParser {

    private static final Pattern CNL_PATTERN =
            Pattern.compile(
                    "Every " + "(a |an )?" + SUBJECT_REGEX + " must " + PHRASES_REGEX + "\\.");

    protected ExistentialRuleParser(
    ...
    protected RuleVisualizer getRuleVisualizer() throws MappingToUmlTranslationFailedException {
        return new ExistentialRuleVisualizer(
                cnlString,
                subjectTriplets,
                verbPhrases,
                conceptManager,
