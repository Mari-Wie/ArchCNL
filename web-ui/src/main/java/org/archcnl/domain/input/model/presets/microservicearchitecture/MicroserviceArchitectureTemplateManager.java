...
/**
 * Template Manager that creates CustomConcepts and CustomRelations for the
...
public class MicroserviceArchitectureTemplateManager implements ArchitecturalStyleTemplateManager {
	...
	/**
	 * Creates the CustomConcept and respective ConceptMaping for the architectural
	 * concept 'Microservice' and uses a regular expression to match all services
	...
	public CustomConcept createMicroservicesConceptAndMapping(Set<TwoColumnGridEntry> services) {
		...
		try {
			ConceptMapping mapping = new ConceptMapping(classVariable, specificMicroserviceWhenTriplets, microservice);
			microservice.setMapping(mapping);

			return microservice;
		} catch (UnsupportedObjectTypeInTriplet | RelationDoesNotExistException | UnrelatedMappingException e) {
			e.printStackTrace();
		}
		return null;
	}
	...
	private List<AndTriplets> createMicroserviceTriplets(String regexInput) {
		...
		try {
			...
			createdTriplets.add(TripletFactory.createTriplet(classVariable,
			...
			createdTriplets.add(TripletFactory.createTriplet(classVariable,
					model.getRelationManager().getRelationByName("hasName").get(), nameVariable));

			// regex(?name, 'NotificationServiceApplication$')
			createdTriplets.add(TripletFactory.createTriplet(nameVariable,
					model.getRelationManager().getRelationByName("matches").get(), new StringValue(regexInput)));
	...
	/**
	 * Creates the CustomConcept and respective ConceptMaping for the architectural
	 * concept 'ServiceRegistry' and uses a regular expression. The regular
	 * expression used will take the provided String and produce an expression of
	 * "(\\w||\\W)*%CLASS_NAME$" where CLASS_NAME is the provided param String of
	 * the method.
	 *
	...
	/**
	 * Creates the CustomRelation 'registerin' including the mapping for the
	 * relation.
	 *
	 * @param registryImportName The name of the class that is imported when
	...
	public CustomRelation createRegisterinRelationAndMapping(String registryImportName) {
		...
		CustomRelation registerinRelation = new CustomRelation("registerin", "", new LinkedList<>());

		List<AndTriplets> whenTriplets = createRegisterinWhenTriplets(regexInput);

		try {
			RelationMapping mapping = new RelationMapping(
					TripletFactory.createTriplet(classVariable, registerinRelation, registryVariable), whenTriplets);
	...
	private List<AndTriplets> createRegisterinWhenTriplets(String regexInput) {
		...
		// (?class rdf:type famix:FamixClass)
		try {
			createdTriplets.add(TripletFactory.createTriplet(registryVariable,
					model.getRelationManager().getRelationByName("is-of-type").get(),
			...
			createdTriplets.add(TripletFactory.createTriplet(registryImportVariable,
					model.getRelationManager().getRelationByName("is-of-type").get(),
					model.getConceptManager().getConceptByName("FamixClass").get()));

			// (?class famix:imports ?registryImport)
			createdTriplets.add(TripletFactory.createTriplet(classVariable,
					model.getRelationManager().getRelationByName("imports").get(), registryImportVariable));

			// (?registryImport famix:hasName ?registryImportName)
			createdTriplets.add(TripletFactory.createTriplet(registryImportVariable,
					model.getRelationManager().getRelationByName("hasName").get(), registryImportNameVariable));

			// e.g. regex(?name, '(\\w||\\W)*RegistryApplication$')
			createdTriplets.add(TripletFactory.createTriplet(registryImportNameVariable,
					model.getRelationManager().getRelationByName("matches").get(), new StringValue(regexInput)));
		} catch (UnsupportedObjectTypeInTriplet e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	/**
	 * Creates the CustomConcept and respective ConceptMaping for the architectural
	 * concept 'DatabaseAccessAbstraction' and uses a regular expression. The
	 * regular expression used will take the provided String and produce an
	 * expression of "(\\w||\\W)*%CLASS_NAME_ENDING$" where CLASS_NAME_ENDING is the
	 * class name nending for the DB-Access-Abstraction. <br>
	 *
	 * <p>
	 * If e.g. a Spring Repository is used the ending (depending on the naming
	 * convention) could be 'Repository' which would then be used as
	 * CLASS_NAME_ENDING in the regular expression. If multiple
	 * DB-Access-Abstractions are used they will be piped within the regex.
	 *
	 * @param serviceregistryClassName the name of the service registry class.
	 */
	public CustomConcept createDbAccessAbstractionConceptAndMapping(Set<TwoColumnGridEntry> set) {
		...
		for (Iterator<TwoColumnGridEntry> iterator = set.iterator(); iterator.hasNext();) {
			...
			String dbAccessAbstractionRegExpPart = String.format("%s$", dbAccessAbstraction.getInfo2());
		...
		List<AndTriplets> dbAccessAbstractionWhenTriplets = createDbAccessAbstractionTriplets(regexExp.toString());

		try {
			ConceptMapping mapping = new ConceptMapping(classVariable, dbAccessAbstractionWhenTriplets, abstraction);
	...
	private List<AndTriplets> createDbAccessAbstractionTriplets(String regexInput) {
		...
		try {
			...
			createdTriplets.add(TripletFactory.createTriplet(classVariable,
					model.getRelationManager().getRelationByName("isExternal").get(), new BooleanValue(false)));
	 ...
	 * <p>
	 * <b>NOTE</b><br>
	 * The order of the triplets is important to avoid "out of memory"-exceptions
	 * when executing the archcnl-toolchain.
	 *
	 * <p>
	 */
	public CustomRelation createHaveownRelationAndMapping() {
		...
		try {
			...
			createdTriplets.add(TripletFactory.createTriplet(class2Variable,
					model.getRelationManager().getRelationByName("is-of-type").get(),
					model.getConceptManager().getConceptByName("FamixClass").get()));

			// (?classPackage rdf:type famix:Namespace)
			createdTriplets.add(TripletFactory.createTriplet(classPackage,
					model.getRelationManager().getRelationByName("is-of-type").get(),
					model.getConceptManager().getConceptByName("Namespace").get()));

			// (?classPackage famix:namespaceContains ?class1)
			createdTriplets.add(TripletFactory.createTriplet(classPackage,
					model.getRelationManager().getRelationByName("namespaceContains").get(), classVariable));

			// (?class2Package rdf:type famix:Namespace)
			createdTriplets.add(TripletFactory.createTriplet(class2Package,
					model.getRelationManager().getRelationByName("is-of-type").get(),
					model.getConceptManager().getConceptByName("Namespace").get()));

			// (?classPackage famix:namespaceContains ?class2Package)
			createdTriplets.add(TripletFactory.createTriplet(class2Package,
					model.getRelationManager().getRelationByName("namespaceContains").get(), class2Variable));

			// (?classPackage famix:namespaceContains ?class2Package)
			createdTriplets.add(TripletFactory.createTriplet(classPackage,
					model.getRelationManager().getRelationByName("namespaceContains").get(), class2Package));
		...
		try {
			RelationMapping mapping = new RelationMapping(
					TripletFactory.createTriplet(classVariable, haveown, class2Variable), haveownWhenTriplets);
	...
	/**
	 * Creates the CustomConcept and respective ConceptMaping for the architectural
	 * concept 'ApiGateway' and uses a regular expression. The regular expression
	 * used will take the provided String and produce an expression of
	 * "(\\w||\\W)*%PACKAGE_NAME$" where PACKAGE_NAME is the provided param String
	 * of the method.
	 *
	 * @param apiGatewayPackageName the name of the package that includes the api
	 *                              gateway.
	 */
	...
	/**
	 * Creates the CustomConcept and respective ConceptMaping for the architectural
	 * concept 'MicroserviceApp' and uses a regular expression. The regular
	 * expression used will take the provided String and produce an expression of
	 * "(\\w||\\W)*%MS_APP_NAME" where PACKAGE_NAME is the provided param String of
	 * the method.
	 *
	 * @param msAppPackageStructure the start (e.g. com.example) of the package
	 *                              structure for the microserviec application.
	 */
	...
	/**
	 * Creates the CustomConcept and respective ConceptMaping for the architectural
	 * concept 'CircuitBreaker' and uses a regular expression. The regular
	 * expression used will take the provided String and produce an expression of
	 * "(\\w||\\W)*%CIRCUIT_BREAKER_IMPORT_NAME%" where CIRCUIT_BREAKER_IMPORT_NAME
	 * is the provided param String of the method.
	 *
	 * @param circuitBreakerImportClassName the name of the class thats imported
	 *                                      when a circuit breaker is used.
	 */
	public CustomConcept createCircuitBreakerConceptAndMapping(String circuitBreakerImportClassName) {
		...
		try {
			ConceptMapping mapping = new ConceptMapping(classVariable, whenTriplets, circuitBreaker);
	...
	private List<AndTriplets> createRuntimeEnvironmentWhenTriplets() {
		...
		try {
			...
			createdTriplets.add(TripletFactory.createTriplet(classVariable,
					model.getRelationManager().getRelationByName("definesMethod").get(), methodVariable));

			createdTriplets.add(TripletFactory.createTriplet(methodVariable,
					model.getRelationManager().getRelationByName("hasName").get(), nameVariable));

			createdTriplets.add(TripletFactory.createTriplet(nameVariable,
					model.getRelationManager().getRelationByName("matches").get(), new StringValue("main")));
	...
	public CustomRelation createUseownRelationAndMapping() {
		...
		try {
			...
			createdTriplets.add(TripletFactory.createTriplet(class2Variable,
					model.getRelationManager().getRelationByName("is-of-type").get(),
					model.getConceptManager().getConceptByName("RuntimeEnvironment").get()));

			// (?classPackage rdf:type famix:Namespace)
			createdTriplets.add(TripletFactory.createTriplet(classVariable,
					model.getRelationManager().getRelationByName("definesMethod").get(), class2Variable));
		...
		try {
			RelationMapping mapping = new RelationMapping(
					TripletFactory.createTriplet(classVariable, useown, class2Variable), useownWhenTriplets);
	...
	public CustomRelation createResideInPackageRelationAndMapping() {
		...
		CustomRelation resideInPackage = new CustomRelation("resideinpackage", "", new LinkedList<>());
		...
		try {
			RelationMapping mapping = new RelationMapping(
					TripletFactory.createTriplet(class2Package, resideInPackage, classPackage), resideInWhenTriplets);
	...
	public CustomRelation createUseRelationAndMapping() {
		...
		try {
			...
			// (?package famix:namespaceContains ?package2)
			createdTriplets.add(TripletFactory.createTriplet(classVariable,
					model.getRelationManager().getRelationByName("imports").get(), class2Variable));
		...
		try {
			RelationMapping mapping = new RelationMapping(
					TripletFactory.createTriplet(classVariable, use, class2Variable), useWhenTriplets);
