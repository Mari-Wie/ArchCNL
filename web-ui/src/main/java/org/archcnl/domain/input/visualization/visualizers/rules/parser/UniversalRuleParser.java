...
import org.archcnl.domain.input.visualization.visualizers.rules.UniversalRuleVisualizer;

public class UniversalRuleParser extends RuleParser {

    private static final Pattern CNL_PATTERN =
            Pattern.compile(
                    "Every " + "(a |an )?" + SUBJECT_REGEX + " can-only " + PHRASES_REGEX + "\\.");

    protected UniversalRuleParser(
    ...
    protected RuleVisualizer getRuleVisualizer() throws MappingToUmlTranslationFailedException {
        return new UniversalRuleVisualizer(
                cnlString,
                subjectTriplets,
                verbPhrases,
                conceptManager,
