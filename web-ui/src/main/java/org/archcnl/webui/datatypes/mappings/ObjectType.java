    ...
    public static ObjectType parseObject(String potentialObject) throws NoObjectTypeException {
    	try {
	    	if (potentialObject.matches("'.+'")) {
	        	...
	        	String boolString = AdocIoUtils.getFirstMatch(Pattern.compile("(?<=').+(?=')"), potentialObject);
	            ...
	            try {
					return RulesConceptsAndRelations.getInstance().getConceptManager().getConceptByName(objectName);
