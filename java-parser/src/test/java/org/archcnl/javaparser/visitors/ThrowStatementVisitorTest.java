...
public class ThrowStatementVisitorTest extends GenericVisitorTest<ThrowStatementVisitor> {
	...
	private String pathToExampleClassWithExceptions = GenericVisitorTest.PATH_TO_PACKAGE_WITH_TEST_EXAMPLES
			+ "ExampleClassWithExceptions.java";

	@Test
	public void testFullyQualifiedNames() throws FileIsNotAJavaClassException, FileNotFoundException {
		final CompilationUnit unit = CompilationUnitFactory.getFromPath(pathToExampleClassWithExceptions);

		unit.accept(visitor, null);

		final List<String> exceptions = visitor.getThrownExceptionType().stream().map(Type::getName)
	...
	public void testSimpleNames() throws FileIsNotAJavaClassException, FileNotFoundException {
		...
		final List<String> exceptions = visitor.getThrownExceptionType().stream().map(Type::getSimpleName)
				.collect(Collectors.toList());
